...
Document::Document(const QByteArray &data): mData(data)
{
}
...
bool Document::open() {
    ...
    {
        Kzip = new KZipStream(mFileName);
    }
    else
    {
        Kzip = new KZipStream(mData);
    }
    qDebug()<<"Kzip stream size:"<<Kzip->stream().size();
    /*if (Kzip->canread())*/ {//FIXME: fail
