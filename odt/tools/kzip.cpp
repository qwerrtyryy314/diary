...
KZipStream::KZipStream(const QByteArray &data)
{
    d = new QBuffer();
    ErrorCode ec = ReadFailed;
    uBuffer = (unsigned char*) buffer1;
    ...
    if (d->open(QIODevice::ReadWrite)) {
        ...
        is_open = true;
        d->write(data);
        d->seek(0);//start(); at loadfile
        //FIXME: write to db/file is fucked
        qDebug()<<"D_DATA:"<<d->data().size();
        ec = seekToCentralDirectory();
        ////qDebug() << "### seekToCentralDirectory return ...  " << ec;
        if (ec != KZipStream::OkFunky) {
...
//NOTE: may be needed for proper zipping?
//        qint64 read;
//        char* p = 0;
